...
pub async fn run_migrations(pool: &Pool) -> Result<(), mysql_async::Error> {
    ...
    let _ = conn.exec_drop(
        r"CREATE TABLE IF NOT EXISTS mc_scheduled_ops (
            id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
...
mod tests {
    ...
    #[test]
    fn create_scheduled_absolute_query_uses_placeholders() {
        let static_query = "INSERT INTO mc_scheduled_ops (scheduled_at, op_type, target, params_json, status) VALUES (?, ?, ?, ?, 'pending')";
        assert_eq!(static_query.matches('?').count(), 4);
    }

    #[test]
    fn create_scheduled_relative_query_uses_placeholders() {
        let static_query = "INSERT INTO mc_scheduled_ops (scheduled_at, op_type, target, params_json, status) VALUES (DATE_ADD(NOW(), INTERVAL ? MINUTE), ?, ?, ?, 'pending')";
        assert_eq!(static_query.matches('?').count(), 4);
    }

    #[test]
    fn cancel_scheduled_query_only_cancels_pending() {
        let static_query = "UPDATE mc_scheduled_ops SET status='cancelled' WHERE id=? AND status='pending'";
        assert!(static_query.contains("status='pending'"));
        assert_eq!(static_query.matches('?').count(), 1);
    }
}
...
#[derive(Debug, Clone)]
/// A scheduled operation from `mc_scheduled_ops`.
/// `op_type` is one of `update_agent|bulk_update_group|restart_gateway|config_push`.
...
pub struct ScheduledOp {
    ...
    pub op_type: String,
    pub target: String,
    /// Optional JSON payload for operation-specific parameters.
